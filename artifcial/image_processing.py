...
def correct_perspective(image_input):
    ...
    print("[OK] [Perspective] Koreksi perspektif berhasil (robust mode).")
    return warped
...
def correct_rotation_hough(image_input):
    ...
    if lines is None:
        print("[WARNING] [Rotation] Tidak ada garis terdeteksi. Melewatkan koreksi rotasi.")
        return image
    ...
    if not angles:
        print("[WARNING] [Rotation] Tidak ada garis horizontal valid. Melewatkan koreksi rotasi.")
        return image

    median_angle = np.median(angles)
    print(f"[OK] [Rotation] Sudut kemiringan terdeteksi: {median_angle:.2f} derajat. Mengoreksi...")
...
def detect_answer_boxes(image_input, max_boxes=4, visualize=False):
    ...
    if not contours:
        print("[WARNING] [Boxes] Tidak ada kontur valid ditemukan.")
        return []
    ...
    print(f"[OK] [Boxes] Jumlah kotak terdeteksi: {len(boxes_detected)}")
